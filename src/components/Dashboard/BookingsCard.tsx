...
const BookingsCard = () => {
  ...
  const filteredData = useMemo(() => {
    const gymBookings = mockData.filter(
      (booking) => booking.gym_name === currentGym
    );

    if (activeTab === 'today') {
      return gymBookings.filter(booking => 
    ...
    return selectedDate
      ? gymBookings.filter(booking =>
          dayjs(booking.date).isAfter(today) &&
          dayjs(booking.date).isSame(dayjs(selectedDate), "day")
        )
      : gymBookings.filter(booking =>
          dayjs(booking.date).isAfter(today)
        );
  }, [activeTab, selectedDate, today, currentGym]);
              ...
              {filteredData.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                    No bookings found for {activeTab === 'today' ? 'today' : 'selected date'}.
                  </td>
                </tr>
              )}
            </tbody>
